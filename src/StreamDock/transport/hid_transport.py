...
_hidapi = None
for lib_name in ['libhidapi-libusb.so.0', 'libhidapi-libusb.so', 'libhidapi.so.0', 'libhidapi.so']:
    try:
...
if _hidapi is None:
    raise ImportError("Could not load libhidapi-libusb. Please install hidapi with libusb backend.")
        ...
        try:
            ...
            if result > 0:
                ...
                if len(result_bytes) > 0:
                    ...
                    ok_response = result_bytes[5:7].decode('utf-8', errors='ignore') if len(result_bytes) > 6 else ''
