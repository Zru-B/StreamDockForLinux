...
class MockTransport:
    ...
    def open(self, path: bytes) -> int:
        ...
        self.logger.info("Opening mock device at %s", path)
    ...
    def read_(self, unused_length: int, timeout_ms: int = -1) -> Optional[Tuple[bytes, str, str, int, int]]:
        """
    ...
    def _write_packet(self, packet: bytearray) -> int:
        ...
        if cmd == 'LIG':
            self.logger.info("[Mock] Set Brightness: %d%%", packet[11])
        elif cmd == 'CLE':
            self.logger.info("[Mock] Clear Key: %d (255=All)", packet[12])
        elif cmd == 'DIS':
            ...
            target = packet[13]
            self.logger.info("[Mock] Set Image. Target: %d (1=BG, >1=Key)", target)
    ...
    def enumerate(self, unused_vid: int, unused_pid: int) -> List[Dict]:
    ...
    def set_key_img(self, unused_path: bytes, key: int) -> int:
        # Just log it
        packet = bytearray(self.PACKET_SIZE)
        packet[6:9] = b'LOG'
        packet[13] = key
    ...
    def set_background_img_from_file(self, unused_path: bytes) -> int:
        packet = bytearray(self.PACKET_SIZE)
        packet[6:9] = b'LOG'
        packet[13] = 1 # BG
        return 1 if self._write_packet(packet) > 0 else -1
