...
def _to_native_format(image, image_format):
    if (
        image_format["format"].lower() != "jpeg"
        and image_format["format"].lower() != "jpg"
    ):
        raise ValueError(
            f"no support format: {image_format['format']}. only 'jpeg' or 'jpg' is supported"
        )

    _expand = True
    if (
        image.size[1] == image_format["size"][0]
        and image.size[0] == image_format["size"][1]
    ):
        _expand = False
...
def create_scaled_touchscreen_image(
    dock, image, margins=[0, 0, 0, 0], background="black"
):
