...
from PyQt6.QtGui import QAction, QColor, QDrag, QFont, QPainter, QPalette, QPixmap
...
class LayoutListWidget(QWidget):
    ...
    def _show_context_menu(self, position):
        ...
        edit_action.triggered.connect(
            lambda: self.edit_layout_clicked.emit(layout_name)
        )
        ...
        set_default_action.triggered.connect(
            lambda: self.set_default_clicked.emit(layout_name)
        )
        ...
        delete_action.triggered.connect(
            lambda: self.delete_layout_clicked.emit(layout_name)
        )
...
class ActionListItem(QWidget):
    ...
    def _format_action(self) -> str:
        ...
        if action_type == "EXECUTE_COMMAND":
            ...
            preview = (
                action_value[:30] + "..." if len(action_value) > 30 else action_value
            )
