...
class KeySelectionDialog(QMessageBox):
    ...
    def __init__(self, available_keys: list, parent=None):
        ...
        self.select_btn = self.addButton(
            "Select Existing", QMessageBox.ButtonRole.ActionRole
        )
        self.create_btn = self.addButton(
    ...
    def select_existing_key(self):
        ...
        if not self.available_keys:
            QMessageBox.warning(
                self, "No Keys", "No existing keys available. Create a new one."
            )
            return

        key_name, ok = QInputDialog.getItem(
            self, "Select Key", "Choose a key:", self.available_keys, 0, False
        )
...
class KeyActionDialog(QMessageBox):
    ...
    def __init__(self, parent=None):
        ...
        self.replace_btn = self.addButton(
            "Replace with Existing", QMessageBox.ButtonRole.ActionRole
        )
        self.create_btn = self.addButton(
            "Create & Replace", QMessageBox.ButtonRole.ActionRole
        )
        self.remove_btn = self.addButton(
            "Remove from Layout", QMessageBox.ButtonRole.ActionRole
        )
        self.cancel_btn = self.addButton("Cancel", QMessageBox.ButtonRole.RejectRole)
...
class ConfigEditorMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        grid_title.setStyleSheet(
            f"color: {get_colors()['text_primary']}; margin-bottom: 12px;"
        )
    ...
    def update_window_title(self):
        ...
        if self.config_file_path:
            title = (
                f"StreamDock Configuration Editor - {Path(self.config_file_path).name}"
            )
        else:
    ...
    def edit_layout(self, layout_name: str):
        ...
        existing_layouts = [
            name for name in self.config.layouts.keys() if name != layout_name
        ]
                ...
                for action in action_list:
                    if "CHANGE_LAYOUT" in action:
                        ...
                        if isinstance(layout_value, str) and layout_value == old_name:
                            ...
                            isinstance(layout_value, dict)
                            and layout_value.get("layout") == old_name
                        ):
    ...
    def delete_layout(self, layout_name: str):
        ...
        if keys_with_actions:
            warning_parts.append(
                f"Keys with CHANGE_LAYOUT actions: {', '.join(keys_with_actions)}"
            )
    ...
    def _key_has_layout_reference(
        self, key_def: KeyDefinition, layout_name: str
    ) -> bool:
        ...
        for action in all_actions:
            if "CHANGE_LAYOUT" in action:
                ...
                if isinstance(layout_value, str) and layout_value == layout_name:
                    ...
                    isinstance(layout_value, dict)
                    and layout_value.get("layout") == layout_name
                ):
                    return True
        return False
    ...
    def on_key_square_clicked(self, position: int):
        ...
        if not self.current_layout:
            QMessageBox.warning(
                self, "No Layout", "Please select or create a layout first."
            )
            return
    ...
    def handle_empty_square_click(self, position: int, square: KeySquare):
        ...
        create_action.triggered.connect(
            lambda: self.create_and_assign_key(position, square)
        )
        ...
        if available_keys:
            ...
            for key_name in available_keys:
                ...
                assign_action.triggered.connect(
                    lambda checked, k=key_name: self.assign_key_to_position(
                        position, square, k
                    )
                )
    ...
    def handle_filled_square_click(self, position: int, square: KeySquare):
        ...
        create_replace_action.triggered.connect(
            lambda: self.create_and_assign_key(position, square)
        )
        ...
        remove_action.triggered.connect(
            lambda: self.remove_key_from_position(position, square)
        )
    ...
    def edit_key(self, key_name: str):
        ...
        editor = KeyEditorDialog(
            key_def, existing_keys, available_layouts, available_keys, self
        )
    ...
    def replace_key(self, position: int, square: KeySquare):
        ...
        key_name, ok = QInputDialog.getItem(
            self, "Replace Key", "Select replacement key:", available_keys, 0, False
        )
    ...
    def add_window_rule(self):
        ...
        if not self.config.layouts:
            QMessageBox.warning(
                self,
                "No Layouts",
                "Create at least one layout before adding window rules.",
            )
            return
        ...
        dialog = WindowRuleDialog(
            available_layouts, existing_rules=existing_rules, parent=self
        )
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if rule_data["name"] in self.config.window_rules:
                QMessageBox.warning(
                    self, "Error", "A rule with this name already exists!"
                )
                return
    ...
    def edit_window_rule(self, rule_name: str):
        ...
        available_layouts = list(self.config.layouts.keys())
        existing_rules = [
            name for name in self.config.window_rules.keys() if name != rule_name
        ]
