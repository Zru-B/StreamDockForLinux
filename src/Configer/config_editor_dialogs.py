...
def create_styled_button(
    text: str, icon: str = None, primary: bool = False
) -> QPushButton:
...
class KeyEditorDialog(QDialog):
    ...
    def __init__(
        self,
        key_def: KeyDefinition = None,
        existing_keys: list = None,
        available_layouts: list = None,
    ...
    def setup_ui(self):
        ...
        self.press_actions_widget = ActionEditorWidget(
            self.available_layouts, self.available_keys
        )
        self.tabs.addTab(self.press_actions_widget, "On Press Actions")

        self.release_actions_widget = ActionEditorWidget(
            self.available_layouts, self.available_keys
        )
        self.tabs.addTab(self.release_actions_widget, "On Release Actions")

        self.double_press_actions_widget = ActionEditorWidget(
            self.available_layouts, self.available_keys
        )
    ...
    def select_icon(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Icon", "", "Images (*.png *.jpg *.jpeg *.gif *.svg *.bmp)"
        )
    ...
    def load_key_data(self):
        ...
        self.double_press_actions_widget.set_actions(
            self.key_def.on_double_press_actions
        )
...
class ActionEditorWidget(QWidget):
    ...
    def __init__(
        self, available_layouts: list = None, available_keys: list = None, parent=None
    ):
        super().__init__(parent)
        self.actions = []
        self.available_layouts = available_layouts or []
    ...
    def edit_action(self, index: int):
        ...
        if 0 <= index < len(self.actions):
            dialog = ActionDialog(
                self.actions[index], self.available_layouts, self.available_keys
            )
            if dialog.exec() == QDialog.DialogCode.Accepted:
                action = dialog.get_action()
                if action:
    ...
    def refresh_keys_list(self):
        ...
        for key_name, key_def in self.config.keys.items():
            ...
            edit_btn.clicked.connect(
                lambda checked, n=key_name: self.edit_key_by_name(n)
            )
            ...
            delete_btn.clicked.connect(
                lambda checked, n=key_name: self.delete_key_by_name(n)
            )
...
class WindowRuleDialog(QDialog):
    ...
    def validate_and_accept(self):
        ...
        if name != self.original_rule_name and name in self.existing_rules:
            QMessageBox.warning(
                self, "Validation Error", f"Rule name '{name}' already exists."
            )
            return

        if not window_name:
            QMessageBox.warning(
                self, "Validation Error", "Window name pattern is required."
            )
            return
...
class LayoutEditorDialog(QDialog):
    ...
    def setup_ui(self, layout_name: str, clear_all: bool):
        ...
        self.clear_all_check = QCheckBox(
    ...
    def validate_and_accept(self):
        ...
        if name != self.original_name and name in self.existing_layouts:
            QMessageBox.warning(
                self, "Validation Error", f"Layout name '{name}' already exists."
            )
            return
...
class AdvancedSettingsDialog(QDialog):
    ...
    def get_settings(self) -> dict:
        ...
        return {"double_press_interval": self.interval_spin.value()}
